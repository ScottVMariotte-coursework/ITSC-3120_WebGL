...
class CameraBounded{
	constructor(pos,fov,aspect,bound,primary){
		this.bound = vec3(100,bound*3,bound*9);
	...
	rotateOrigin(r){
		console.log(r);
		this.pos = vec3(mult(r,vec4(this.pos)));
	}
	...
	move(xyz) {
		...
		this.pos = vec3(this.pos[0]+(xyz[0]*s),
						this.pos[1]+(xyz[1]*s*5),
						this.pos[2]+(xyz[2]*s));
		...
		if(Math.abs(this.pos[0]) > d){
			this.pos[0] = d * (this.pos[0]/Math.abs(this.pos[0]));
		}
		if(Math.abs(this.pos[2]) > d){
			this.pos[2] = d * (this.pos[2]/Math.abs(this.pos[2]));
		}
	}
	//Set Up for bounded movment
	move_fps(xyz) {
