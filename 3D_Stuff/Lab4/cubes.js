...
window.onload = function init() {
	...
	//v_TriBuffer = gl.createBuffer();
	//vc_TriBuffer = gl.createBuffer();
	...
	//gl.bindBuffer(gl.ARRAY_BUFFER, v_TriBuffer);
	//gl.bufferData(gl.ARRAY_BUFFER, flatten(tri_verts), gl.STATIC_DRAW);
	
	//gl.bindBuffer( gl.ARRAY_BUFFER, vc_TriBuffer);
...
function render(){
	...
	//Create base roation matrix
	M_cube = rotate(theta,axes);
	theta+=2;
	...
	//-------------Draw Cone--------------
	//Mulitiply base rotation matrix by new matrix to rotate by y
	M_cube = mult(M_cube,rotate(tri_theta,[0,1,0]));
	tri_theta += 5;
	
	gl.uniformMatrix4fv(M_Loc, false, flatten(M_cube));
	gl.drawArrays(gl.TRIANGLES, 36, cube_verts.length );
