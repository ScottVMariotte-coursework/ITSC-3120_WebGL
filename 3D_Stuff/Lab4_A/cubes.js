...
function render(){
	...
	//-------------Draw Top Left--------------
	M_cube = identity4();
	var rot, transl_origin,transl, M_cube, M_tetra, M;
	
	transl_origin = translate(.5,-.5,-.5);
	scale = scale4x4(.5,.5,.5);
	rot = rotate(theta,[1,0,0]);
	transl = translate(-.5,.5,.5)

	scale.matrix = true;

	M_cube = mult(transl,mult(rot,mult(scale,transl_origin)));
	
	gl.uniformMatrix4fv(M_Loc, false, flatten(M_cube));
	gl.drawArrays(gl.TRIANGLES, 0, 36);
	
	//-------------Draw Bot Left--------------
	scale = scale4x4(1,1,1);
	rot = rotate(theta,[0,1,0]);
	transl = translate(.5,.5,.5)
...
function getCubeVertices() {
	origin = [-.5,.5,.5]
	return [
        vec3( -0.2+origin[0], -0.2+origin[1],  0.2+origin[2] ),
        vec3( -0.2+origin[0],  0.2+origin[1],  0.2+origin[2] ),
        vec3(  0.2+origin[0],  0.2+origin[1],  0.2+origin[2] ),
        vec3(  0.2+origin[0], -0.2+origin[1],  0.2+origin[2] ),
        vec3( -0.2+origin[0], -0.2+origin[1], -0.2+origin[2] ),
